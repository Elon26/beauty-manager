...
export function DuringFastingModal() {
  const { closeModal } = useModals();
  const setIsDuringFastingModalNeedToShow = useSetStorage(
    'isDuringFastingModalNeedToShow'
  );
  const [isDontShowAgain, setIsDontShowAgain] = useState(true);
  const { width, height } = useWindowDimensions();
  const points = [
    t('pages.fasting-tracker.during-fasting-text-first'),
    t('pages.fasting-tracker.during-fasting-text-second'),
  ];

  function handleContinue() {
    if (isDontShowAgain) {
      setIsDuringFastingModalNeedToShow(false);
    }
    closeModal('DuringFastingModal');
  }
    ...
    <View className="items-center justify-center" style={{ width, height }}>
      ...
      >
        <UiText className="text-center text-lg font-medium">
          {t('pages.fasting-tracker.during-fasting-title')}
        </UiText>
